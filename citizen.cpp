...
void CNPC_Citizen::Spawn()
{
	...
	// [MODIFICATION-Dynamic Cover] - Initialize variables
	m_hLowCoverProp = NULL;
	m_hHighCoverProp = NULL;
	m_hLastArrivedProp = NULL;
	m_flNextPropSearchTime = 0.0f;
...
// ============================================
int CNPC_Citizen::CountNPCsUsingProp(CBaseEntity* pProp)
{
	if (!pProp)
		return 0;
...
bool CNPC_Citizen::DynamicPropCover()
{
...
void CNPC_Citizen::GatherConditions()
{
	...
	if (GetState() == NPC_STATE_COMBAT && GetEnemy())
	{
		...
		{
			ClearCondition(COND_CIT_HAS_LOW_COVER);
			ClearCondition(COND_CIT_HAS_HIGH_COVER);
		...
		else
		{
			...
			if (m_hLowCoverProp)
			{
				...
				{
					SetCondition(COND_CIT_HAS_LOW_COVER);
			...
			if (m_hHighCoverProp)
			{
				...
				if (!enemyNPCNearCover && GetNavigator()->CanFitAtPosition(testPos, MASK_NPCSOLID))
				{
					SetCondition(COND_CIT_HAS_HIGH_COVER);
...
int CNPC_Citizen::SelectSchedule()
{
	...
	{
		bool hasLow = HasCondition(COND_CIT_HAS_LOW_COVER);
		bool hasHigh = HasCondition(COND_CIT_HAS_HIGH_COVER);
		...
		if (hasLow && hasHigh)
		{
			...
			if (lowValid && highValid)
			{
				...
				return useLowCover ? SCHED_CITIZEN_MOVE_TO_LOW_COVER : SCHED_CITIZEN_MOVE_TO_HIGH_COVER;
			}
			else if (lowValid)
			{
				Msg("[DEBUG] SelectSchedule: Only LOW COVER in valid range\n");
				return SCHED_CITIZEN_MOVE_TO_LOW_COVER;
			}
			else if (highValid)
			{
				Msg("[DEBUG] SelectSchedule: Only HIGH COVER in valid range\n");
				return SCHED_CITIZEN_MOVE_TO_HIGH_COVER;
			}
			else
			{
...
void CNPC_Citizen::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_CIT_MOVE_TO_HIGH_COVER:  // [MODIFICATION-Dynamic Cover]
	{
	...
	case TASK_CIT_MOVE_TO_LOW_COVER: // [MODIFICATION-Dynamic Cover]
	{
...
AI_BEGIN_CUSTOM_NPC( npc_citizen, CNPC_Citizen )
	...
	DECLARE_TASK(TASK_CIT_MOVE_TO_HIGH_COVER) // [MODIFICATION-Dynamic Cover]
	DECLARE_TASK(TASK_CIT_MOVE_TO_LOW_COVER) // [MODIFICATION-Dynamic Cover]
#if HL2_EPISODIC
	...
	DECLARE_CONDITION(COND_CIT_HAS_HIGH_COVER) // [MODIFICATION-Dynamic Cover]
	DECLARE_CONDITION(COND_CIT_HAS_LOW_COVER) // [MODIFICATION-Dynamic Cover]
#ifdef EZ
...
//	"		TASK_PLAY_SEQUENCE_FACE_TARGET		ACTIVITY:ACT_DISARM"
	...
	//=========================================================
	// SCHED_CITIZEN_MOVE_TO_HIGH_COVER - // [MODIFICATION-Dynamic Cover
	//=========================================================
	DEFINE_SCHEDULE
	(
	SCHED_CITIZEN_MOVE_TO_HIGH_COVER,

	"	Tasks"
	"		TASK_STOP_MOVING						0"
	"		TASK_CIT_MOVE_TO_HIGH_COVER			0"
	...
	//=========================================================
	// SCHED_CITIZEN_MOVE_TO_LOW_COVER - // [MODIFICATION-Dynamic Cover
	//=========================================================

	DEFINE_SCHEDULE
	(
	SCHED_CITIZEN_MOVE_TO_LOW_COVER,

	"   Tasks"
	"		TASK_STOP_MOVING					    0"
	"		TASK_CIT_MOVE_TO_LOW_COVER			0"
	"		TASK_RUN_PATH_TIMED						4"
	"		TASK_WAIT_FOR_MOVEMENT					0"
	"		TASK_FACE_ENEMY							0"
	"		TASK_RANGE_ATTACK1						0"
